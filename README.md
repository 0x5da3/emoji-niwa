...
- **Procedural terrain** — 20 presets in 5 categories (Green / Mountain & Snow / Arid & Rock / Water / Built — e.g. grassland, forest, Amazon, snowy peaks, volcano, canyon, atoll, waterfall, city) via Perlin noise, plus solid fill and random scatter
...
- **GIF export** — the “📸” button (below 🔈) records a ~3-second square clip of your garden **at the current view and zoom** (no UI in the frame) and exports it as a self-encoded GIF, via the share sheet or a download. On iOS you can then turn it into a Live Photo with an Apple Shortcut / compatible app (done on-device — browsers can't create Live Photos directly)
- **Share link** — the “🔗” button in the left button column encodes your world into a share link (a static snapshot, no server). Opening one starts a read-but-editable visiting mode that never touches the viewer's own garden or autosave (with “Back to my garden” / “Make this mine”). Distinct from Multiplayer below: this is a one-way snapshot, not live co-editing
- **Multiplayer (optional)** — “👥” issues an invite link for real-time co-editing with friends. **Issuing requires a GitHub-login member**; anyone with the invite link can join. While in a room the “👥” button shows live occupancy (e.g. 2/8), a chat box (3 lines above the palette, collapsible/expandable) lets you talk, joins are announced in chat, and your offline garden / autosave is never touched. Needs the optional backend (Rust/Actix, see `server/`). If unconfigured, offline play and `🔗` share links work as before

...
- **Auto terrain emoji** — generating land also scatters emoji that match each terrain type (grass → trees/flowers, water → fish/ducks, sand → cactus, snow → snowman/deer, rock → rocks/conifers) as decor plus a few wandering animals. The on/off toggle and density slider live right inside the “🗺 Land generation” panel (default on), so generating a map applies them in one step. Generating starts a fresh world (replaces hand-placed emoji)

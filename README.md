...
- **GIF / video export** — the “📸” button (below 🔈) first shows the **exact square area that will be captured** (anchored to the top of the screen, everything outside dimmed); while it's shown you can pan / zoom the garden to frame the shot, then pick **📸 Make GIF** (self-encoded GIF, for the iOS Live Photo flow) or **📹 Make video** (a real video file uploadable to X / social apps — **MP4 (H.264) on Safari/iOS, WebM elsewhere**, 3s, silent, via `MediaRecorder`/`captureStream`). Both save via the system share sheet (pick Share / Save to Photos / etc.; falls back to a direct download on unsupported devices). Resolution is selectable right on the capture frame — Standard 320 / High 720 / Max 1080, default High 720. The post-save hint is per-device: on iOS, convert the GIF to a Live Photo with an Apple Shortcut / compatible app (done on-device — browsers can't create Live Photos directly); on Android, use the GIF in a live-wallpaper / motion-photo app

...
- **多人数（任意・共有プレイ）** — 「👥」で招待 URL を発行し、友達とリアルタイム共同編集。**発行は GitHub ログイン会員のみ**、参加は招待 URL を開けば誰でも可。参加中はパレット上のチャット（3行・最小化/拡大可）で会話でき、自分のオフライン箱庭・オートセーブには干渉しません。要・別バックエンド（Rust/Actix、`server/` 参照）。未設定ならオフライン/`🔗`共有は従来どおり動作

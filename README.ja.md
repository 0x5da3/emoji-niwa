...
- **URL 共有** — 左の縦ボタン列の「🔗」で自分のワールドを URL 化（サーバー不要）。生成直後のマップは「レシピ（生成器＋シード）」として共有するため、50×50 でも短いリンク（約150文字）に収まります。手描きした地形は従来どおり完全スナップショットにフォールバック。開いた人は閲覧モードで、その人の箱庭やオートセーブには干渉しません（「自分の箱庭に戻る」「これを自分のものに」も可能）

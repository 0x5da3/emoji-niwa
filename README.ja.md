...
- **地形に合う絵文字の自動配置** — 土地を生成すると、その地形タイプに合う絵文字（草原→木・花、水→魚・カモ、砂→サボテン、雪→雪だるま・シカ、岩→岩・針葉樹）を装飾＋徘徊する動物として自動配置。オン/オフと密度スライダーは「🗺 土地の生成」パネル内にあり（既定オン）、マップ生成と一括で適用されます。生成は手置きの絵文字を消して新しい世界を作ります
...
- **URL 共有** — 左の縦ボタン列の「🔗」で自分のワールドを URL 化（サーバー不要）。生成直後のマップは「レシピ（生成器＋シード）」として共有するため、50×50 でも短いリンク（約150文字）に収まります。手描きした地形は従来どおり完全スナップショットにフォールバック。自動配置の絵文字はリンクに列挙されず、開いた人が同じシードから再生成するため、密度が高い 50×50 でも約150文字を維持します（動物は決定論的な出現セルから動き始めます）。開いた人は閲覧モードで、その人の箱庭やオートセーブには干渉しません（「自分の箱庭に戻る」「これを自分のものに」も可能）

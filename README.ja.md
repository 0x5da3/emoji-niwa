...
- **GIF 書き出し** — 「📸」ボタン（🔈 の下）で小さなパネルが開き、**縮尺**（拡大率そのまま／今の見たまま／箱庭全体）とズーム微調整スライダーを選んでから、約3秒・正方形のクリップ（UI は写り込みません）を自前エンコードの GIF に書き出し、共有シートまたはダウンロードで保存。iOS では保存後に Apple ショートカット／対応アプリで Live Photo 化できます（端末側で実施。ブラウザからは Live Photo を直接作成できません）
